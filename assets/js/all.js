...
Vue.component('card-component', {
  ...
  computed: {
    statusClass: function statusClass() {
      var status = this.cordPrpos.Status;
  ...
  methods: {
    tempStarClick: function tempStarClick() {
...
var app = new Vue({
  ...
  computed: {
    filterCounty: function filterCounty() {
      var vm = this;
      /*----------  寫法一  ----------*/
      // const county = [];
      ...
      */
      ...
      var reduceData = vm.AJAX_data.reduce(function (reduceObj, _ref, index) {
    ...
    cardsData: function cardsData() {
      ...
      */
      ...
      var filterCountyData = AJAX_data.filter(function (_ref2) {
        var County = _ref2.County,
      ...
      var unfilterCountyData = AJAX_data.filter(function (_ref3) {
        var County = _ref3.County,
    ...
    staredData: function staredData() {
      var AJAX_data = this.AJAX_data,
          stared = this.stared;
      var filterStaredData = AJAX_data.filter(function (_ref4) {
  ...
  methods: {
    getAjaxData: function getAjaxData() {
      var vm = this;
      var proxyServerPath = 'https://cors-anywhere.herokuapp.com/';
      var openDataApiPath = 'http://opendata2.epa.gov.tw/AQI.json';
      var apiUrl = proxyServerPath + openDataApiPath;
      $.get(apiUrl).then(function (response, status, xhr) {
        ...
        if (xhr.readyState === 4) {
          ...
          setTimeout(function () {
            // 讓讀取動畫基本要跑 2 秒
            vm.isLoading = false;
          }, 2000); // vm.getLocalStorageStaredDB(response);
        }

        ;
      });
    },
    cardStarClick: function cardStarClick(item) {
      var vm = this; // console.log('cardStarClick-item', item);

      console.log('cardStarClick() - item.SiteName', item.SiteName);
      var siteNameIndex = vm.stared.indexOf(item.SiteName);
      ...
      */
      ...
      siteNameIndex >= 0 ? vm.stared.splice(siteNameIndex, 1) : vm.stared.push(item.SiteName); // 將點按事件所加入的資料 VueJS 實體內的 stared 陣列，觸發後同時將資料寫入 localStorage 裡。

      vm.saveStaredToLocalStorage();
    },
    saveStaredToLocalStorage: function saveStaredToLocalStorage() {
      var vm = this;
      localStorage.setItem(vm.staredToLocalStoragePath, JSON.stringify(vm.stared));
    },
    getLocalStorageStaredDB: function getLocalStorageStaredDB(ajaxDataResponse) {
      var vm = this;
      var localStorageData = JSON.parse(localStorage.getItem(vm.staredToLocalStoragePath));
      /*----------  寫法一  ----------*/
